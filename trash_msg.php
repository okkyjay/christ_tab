...
if(isset($_POST['delSel']))
{
	...
	$resp = deleteTrashMax($mid, $uid);
	$resp['title'] = 'Delete Trashed Message';
	echo json_encode($resp);
	
	exit();
}

if(isset($_POST['restSel']))
{
	$mids = '';
	...
	//echo $mid;
	$resp = untrashInboxMax($mid, $uid);
	$resp['title'] = 'Restore Message';
	echo json_encode($resp);
	exit();
}
...
$query_rec = "SELECT a.mid, a.uid as uid_int, a.date_trashed, a.is_read, (SELECT concat(sname, ', ', fname) AS sender FROM mail_users WHERE id = b.uid) AS sender, b.uid, b.subject, b.body FROM (inbox_msg AS a INNER JOIN messages AS b ON a.mid = b.id) WHERE a.uid = ".$_SESSION['uid_int']." AND trash = 1 ORDER BY date_trashed DESC";
...
<div class="container span 10">
    ...
    <div class="span3">
      <div class="well sidebar-nav">
        <ul class="nav nav-list">
          ...
          <li class="active"><a href="trash_msg.php"><span class="icon-trash"></span>Trash</a></li>
        </ul>
      </div>
      <!--/.well -->
    </div>
  ...
  <div class="span9">
    ...
    <div class="row-fluid">
	  ...
	  if($totalRows_rec == 0)
	  {
		  echo "<em>There is no Message in your Trash Folder</em>";
	  }
	  else
	  {?>
          ...
          <table border="0" width="800px" height="132" class="table table-bordered">
            ...
            <?php do { ?>
              <tr>
                ...
                <td valign="middle"><a href="view_msg.php?mid=<?php echo md5($row_rec['mid']);?>&rid=<?php echo md5($row_rec['uid_int']);?>&folder=trash" style="<?php if($row_rec['is_read'] == 0)echo "font-weight:bold"; ?>"><?php echo $row_rec['subject']; ?></a></td>
                <td valign="middle"><?php echo $row_rec['date_trashed']; ?></td>
              </tr>
              ...
              <tfoot>
                  <tr>
                    ...
                    <input name="delSel[]" type="submit" class="btn-danger" id="delSel2" value="Delete" />
                    <input name="restSel[]" type="submit" class="btn-primary" id="restSel2" value="Restore to Inbox" />
                    </strong></td>
