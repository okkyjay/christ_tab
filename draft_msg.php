...
$query_rec = "SELECT (SELECT concat(sname, ', ', fname) AS sender FROM mail_users WHERE id = uid) AS sender, uid, id as mid, is_read, subject, body, date_created FROM messages WHERE uid = ".$_SESSION['uid_int']." AND sent = 0 ORDER BY date_created DESC";
          ...
          <table border="0" width="800px" height="132" class="table table-bordered">
            ...
            <?php do { ?>
              <tr>
                ...
                <td valign="middle"><a href="compose_msg.php?action=draft&mid=<?php echo md5($row_rec['mid']);?>&d=<?php echo $row_rec['mid'];?>&rid=<?php echo md5($row_rec['uid']);?>" style="<?php if($row_rec['is_read'] == 0)echo "font-weight:bold"; ?>"><?php echo $row_rec['subject']; ?></a></td>
